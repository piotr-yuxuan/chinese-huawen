...
;; Actually the Id problem accepts more than one solution. We choose to change the data structure (IDC vs. :Version) but remember we must address it properly. Indeed it won't be trivial to deal with nested diffrent versions. Another solution is to keep data homogeneous: the parsed collection returned contains one or 2*i elements. If one then no problem; if 2*i, then 0 mod 2 are version and 1 mod 2 are version ids. This data structure is more appealing for a Clojure mindset but from my umble current viewpoint, :Version will warn very well the user the latter has to deal with the former.
;; In addition to this, ⿰&CDP-88F0;&CDP-8C4E;	⿰&CDP-8B5E;&CDP-8C4E; is a good counter-example for the flat solution. So basically you just have to count: if the output is counted more than 1, then we'll deal with versions.
;; Same explanation for :Code.
(def grammar
  (insta/parser
   "<S> =       Letter | (Version <Sep*>)+ | Form
    Sep =       '\t' | ' '*
    Version =   Form Id* (* needed because <S> *)
    <Form> =    ⿰ | ⿱ | ⿴ | ⿵ | ⿶ | ⿷ | ⿸ | ⿹ | ⿺ | ⿻ | ⿲ | ⿳
    Id =        <'['> #'[A-Z]+' <']'>
    ⿰ = <'⿰'> IDC2
    ⿱ = <'⿱'> IDC2
